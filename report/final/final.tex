...
\begin{document}
\begin{frontmatter}
\title{Balancing an Inverted Double Pendulum with Reinforcement Learning} % Title of the project.
...
\begin{abstract}
    ...
    Results show that the double pendulum can be balanced for up-to three seconds with a network of $10$ layers and $30$ nodes in each layer.
...
Q-learning is a model free method \cite{original_q_learning} where the agent has no information about the system it is trying to control, thus it can be said that it is a black box model.
...
The weights of the network were initialized with uniform HE-initialization, which is an optimized weight initialization method when using ReLU activation functions \cite{He_initialization}.
\begin{table}[H]
    \centering
    \begin{tabular}{|
...
A comparison between the agents performance, baseline and a random controller is also illustrated in the figure.
