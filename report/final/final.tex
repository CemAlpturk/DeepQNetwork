...
%Note that the agent itself treats the system as a black box and has no knowledge about the system dynamics.
...
%For the double pendulum case, the simulator solves Equation \eqref{eq_rhs_double_pendulum} for $\ddot{y}$ to produce the next state using the \texttt{odeint} function.
...
\begin{flalign}
	r(s) &= 1 - \left( \frac{|\theta_1|}{\theta_{max}} + \frac{|\theta_2|}{\theta_{max}}\right) - \left(\dot{\theta_1}^2 + \dot{\theta_2}^2 \right) \nonumber \\
	R(s) &=
	\begin{cases}
		-10 & \text{ if \eqref{eqn:termination_double_pendulum}} \\
		  \max(0,r(s)) & \text{ otherwise}
	\end{cases} 
...
%, but they did not achieve better performance than our Deep Q-learning \cite{Deep_Q_Learning_First} implementation.
