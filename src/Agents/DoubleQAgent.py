...
class DoubleQAgent:
    ...
    def _build_targets(
        ...
        non_terminal_count = np.sum(i)
        targets = np.zeros((batch_size,len(self.environment.action_space)))
        acts = np.argmax(self.q_network.predict(next_states[i,:]),axis=1)
        ...
        targets[i, actions[i]] += discount * t[range(non_terminal_count),acts] # add next estimation to non terminal rows

        return targets

    def _align_target_model(self):
        tmp = self.q_network.get_weights()
        self.q_network.set_weights(self.target_network.get_weights())
        self.target_network.set_weights(tmp)
